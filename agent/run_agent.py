...
def run_agent_for_repo(
    ...
    with DirContext(repo_path):
        ...
        if getattr(agent_config, "run_one_shot", False):
            ...
            update_queue.put(("start_repo", (repo_name, 1)))
            update_queue.put(("set_current_file", (repo_name, "ALL")))
            ...
            with DirContext(str(work_dir)):
                agent_return = agent.run(message, "", "", [], one_shot_log_dir)
            if use_sandbox:
            ...
            update_queue.put(
                ("update_money_display", (repo_name, "ALL", agent_return.last_cost))
            )
